...
const LocationSection = () => (
  <section id="location" className="py-12 md:py-20 px-4 sm:px-6 overflow-hidden" style={{ background: "var(--section-gradient)" }}>
    <div className="container mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          {highlights.map(({ icon: Icon, text }, idx) => (
            <Reveal key={text} animation="up" delay={idx * 0.1}>
              <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 bg-card rounded-lg shadow-sm border border-border hover:border-primary/30 hover:shadow-md transition-all card-lift">
                <Icon className="text-primary shrink-0" size={18} />
                <span className="text-xs sm:text-sm font-medium text-foreground leading-tight">{text}</span>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6 md:gap-8 items-start mb-8">
        {/* Map */}
        <Reveal animation="left" className="lg:col-span-1">
          <div className="rounded-2xl overflow-hidden shadow-lg h-[220px] sm:h-[300px] lg:h-[400px]">
            <iframe
          ...
          <a
            href="https://maps.google.com/?q=Koheda,Hyderabad,Telangana"
            target="_blank"
            rel="noopener noreferrer"
            className="mt-3 flex items-center justify-center gap-2 w-full py-2.5 px-4 rounded-lg border border-primary/30 text-primary text-sm font-medium hover:bg-primary/5 transition-colors"
          >
            <MapPin size={15} />
            Open in Maps
          </a>
        ...
        <div className="lg:col-span-2">
          <Reveal animation="up">
            <h3 className="font-heading text-xl sm:text-2xl font-semibold text-foreground mb-4 md:mb-6">Nearby Features</h3>
          </Reveal>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            {nearbyFeatures.map(({ icon: Icon, title, distance }, idx) => (
              <Reveal key={title} animation="right" delay={0.05 + idx * 0.08}>
                <div className="p-4 sm:p-5 bg-card rounded-xl border border-border shadow-sm hover:shadow-md hover:border-primary/25 transition-all card-lift h-full">
                  <div className="flex items-start gap-3 sm:gap-4">
                    <div className="p-2 sm:p-2.5 bg-primary/10 rounded-lg text-primary flex-shrink-0">
                      <Icon size={18} />
                    </div>
                    <div>
                      <h4 className="font-heading text-sm sm:text-base font-semibold text-foreground">{title}</h4>
                      <p className="text-xs sm:text-sm text-muted-foreground mt-1">{distance}</p>
                    </div>
                  </div>
                </div>
              </Reveal>
